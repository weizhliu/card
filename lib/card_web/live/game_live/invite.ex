defmodule CardWeb.GameLive.Invite do
  ...
  def mount(%{"id" => id} = _params, _session, socket) do
    ...
    if room = Room.get(id) do
      {:ok,
       assign(socket, %{
         room: room,
         player: socket.assigns.live_action,
         invite_path: url(socket, ~p"/#{id}/guest/invite")
       })}
    else
  ...
  def handle_info(
    ...
    case player do
      :host -> {:noreply, push_navigate(socket, to: ~p"/#{room.id}/host")}
      :guest -> {:noreply, push_navigate(socket, to: ~p"/#{room.id}/guest")}
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col items-center h-screen">
      ...
      <.live_component module={CardWeb.ReadyComponent} player={@player} id={:ready} room={@room}>
        <:title>
          <div class="bg-blue-300 -mt-8 w-32 h-8 rounded-xl transform skew-x-12 -rotate-6 translate-y-6 translate-x-20">
          </div>
  ...
  def invite(assigns) do
    ...
    <div class="flex flex-col p-4 bg-gray-50 shadow-lg text-center mt-12">
      <div class="bg-blue-300 w-40 h-8 rounded-xl transform skew-x-12 rotate-3 -mt-6 translate-y-7 translate-x-44">
      </div>
      <label for="invite_url" class="transform text-2xl font-serif">Guest Link</label>
      <br />
      <div class="flex border-2 rounded-xl bg-white justify-center items-center mt-4">
        <input
          id="invite_url"
          class="w-96 text-center text-gray-500 border-none rounded-l-xl"
          type="text"
          value={@path}
          readonly
        />
        <button
          onclick="navigator.clipboard.writeText(document.querySelector('#invite_url').value)"
          class="btn flex group -rotate-2"
        >
          <div class="bg-green-300 w-16 h-8 rounded-xl rounded-l-none transform -skew-x-2 -ml-16 translate-x-16 border-2 border-white group-hover:border-green-300">
          </div>
