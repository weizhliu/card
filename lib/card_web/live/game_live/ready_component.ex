defmodule CardWeb.ReadyComponent do
      ...
      <%= unless Map.get(@room, :"#{@player}_ready") do %>
        <div class="flex justify-between w-44 mx-auto mt-8">
          ...
          <button phx-click="ready" phx-target={@myself} class="ml-2 h-8 group">
            <div class="border-2 border-white group-hover:border-green-300 bg-green-300 w-10 h-12 rounded-xl transform skew-x-12 -rotate-45 translate-x-1 translate-y-2">
            </div>
  ...
  def ready_status(assigns) do
    ...
    <div class="flex">
      <%= @player %>:
      <div class={"ml-8 #{if @ready, do: "bg-green-300", else: "bg-red-300"} w-6 h-6 rounded-3xl transform -skew-x-3 rotate-12"}>
      </div>
    </div>
    """
  end
